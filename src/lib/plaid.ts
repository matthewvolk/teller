...
const PLAID_API_SECRET = env.PLAID_SANDBOX_API_SECRET;
...
export const accountsGet = async ({ accessToken }: SelectAccount) => {
  if (!accessToken) {
    return null;
  }
...
export const transactionsSync = async ({ accessToken }: SelectAccount) => {
  if (!accessToken) {
    return null;
  }
